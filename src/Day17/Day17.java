...
public class Day17 {
    ...
    private void part1(ArrayList<String> input) {
        char[][][] grid;
        ...
        for (int cycle = 1; cycle <= 6; cycle++) {
            ...
            for (int w = 0; w < grid.length; w++) {
                for (int z = 0; z < grid.length; z++) {
                    for (int y = 0; y < grid.length; y++) {
                        for (int x = 0; x < grid.length; x++) {
                            int activeNeighbors = checkOccupied2(grid, w, z, y, x);
                            if (grid[w][z][y][x] == '#') {
                                if (activeNeighbors == 2 || activeNeighbors == 3) {
                                    copy[w][z][y][x] = '#';
                                } else {
    ...
    private int checkOccupied(char[][][] seats, int i, int j, int k) {
        int count = 0;
    ...
    private int checkOccupied2(char[][][][] seats, int i, int j, int k, int l) {
        int count = 0;
        for (int dx = (i > 0 ? -1 : 0); dx < (i < seats.length - 1 ? 2 : 1); dx++) {
            for (int dy = (j > 0 ? -1 : 0); dy < (j < seats.length - 1 ? 2 : 1); dy++) {
                for (int dz = (k > 0 ? -1 : 0); dz < (k < seats.length - 1 ? 2 : 1); dz++) {
                    for (int dw = (l > 0 ? -1 : 0); dw < (l < seats.length - 1 ? 2 : 1); dw++) {
                        if (dx != 0 || dy != 0 || dz != 0 || dw != 0) {
                            if (seats[i + dx][j + dy][k + dz][l + dw] == '#') {
                                count++;
                            }
                        }
                    }
